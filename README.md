...
| **M5 · 答题链路** | ✅ **完成（已实跑）** | 本地 OpenAI 兼容代理 + 待答工单，操控 Agent 作答；答案自动转选项原文；默认不交卷。实跑：电机与电气控制 2.1/2.2/2.5 三章章节检测全部交卷通过 |
| M3 · 真跑 | ✅ 已完成部分 | 建筑防排烟 27/27 视频真实跑完（电机与电气控制 2.1/2.2/2.5 三章含 quiz 全完成） |
| **当前规模** | — | **293 项测试全绿**；37 条命令；4 个 Adapter（2 真实上游 + mock 兜底 + 3 个已注册待启用） |
| **M6 · 签到** | ✅ **完成（C48–C51）** | 复用 chaoxing-cli 的原语 + 自建编排：`sign_status` 发现进行中签到（实测可用）、`sign_in` 执行（normal 已实现）、`sign_watch` 有界轮询；**签退 C51 未实现（如实标 none）** |
| M4 · 扩展 | ⏸ | 图片题 / 课程资料下载 / MCP 联调 |

> ⚠️ **读侧（xuexitong-mcp）与写侧（chaoxing-cli）已接真实上游**；其余未接入的能力仍走 Mock 兜底，返回内置样本数据。 判断方法：看输出里的 `adapter=` 字段，`adapter=mock` 即假数据。真实上游要等 M1–M3 接入。
...
平台本体**不需要浏览器**——整条链路走 HTTP + 子进程。浏览器只在两个窄场景才用得上：浏览器脚本类能力（`C38`）和签到的位置/二维码/拍照类型（M6 的签到发现与普通签到走 HTTP，不需要浏览器）。
